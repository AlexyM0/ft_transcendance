...
import type { TournamentLite, TournamentPlayerSlot, TournamentMatch, TournamentFull, CreateTournamentPayload, UpdateAliasPayload } from "../types/state_types";
...
export async function listTournaments(req: FastifyRequest, rep: FastifyReply) {
  ...
  const tournaments = tournamentsService.listActiveTournaments(q, status, limit, offset);
...
export async function leaveTournament(req: FastifyRequest, rep: FastifyReply) {
  const userId = Number((req.user as any).sub) as number;
  const tournamentId = Number((req.params as any).tournamentId);
  return rep.code(201).send(tournamentsService.leaveTournament(tournamentId, userId));
}
...
export async function updateAlias(req: FastifyRequest, rep: FastifyReply) {
  ...
  return rep.code(201).send(tournamentsService.updateAlias(tournamentId, userId, payload));
}
...
export async function startTournament(req: FastifyRequest, rep: FastifyReply) {
  ...
  const tournamentId = Number((req.params as any).tournamentId);
  return rep.code(201).send(tournamentsService.startTournament(tournamentId, userId));
}
...
export async function recordTournamentMatchResult(req: FastifyRequest, rep: FastifyReply) {
  ...
  tournamentsService.recordMatchResult(matchId, Number(scoreP1), Number(scoreP2), userId);
