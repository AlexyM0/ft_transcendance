 ...
 * SQL commands
 */
const selectPublicById = db.prepare(`SELECT id, email, pseudo, avatar_url FROM users WHERE id = ?`);
...
const updateProfileQuery = db.prepare(
  `UPDATE users SET email = COALESCE(?, email), pseudo = COALESCE(?, pseudo), avatar_url = COALESCE(?, avatar_url) WHERE id = ? RETURNING id, email, pseudo, is_2fa_enabled, avatar_url`
);
...
const selectAllUsers = db.prepare(`SELECT id, pseudo, avatar_url FROM users`);
...
export function updateMeProfile(userId: number, input: { email?: string; pseudo?: string; avatarUrl?: string }) {
  return updateProfileQuery.get(input?.email ?? null, input?.pseudo ?? null, input?.avatarUrl ?? null, userId) as MeUserRow | undefined;
}
