...
export const UsersAPI = {
  ...
  uploadAvatar: (avatar: File) => {
    ...
    return putForm<Me>("/users/me/avatar", form);
  },

  /**
   * DELETE /api/users/me/avatar
   */
  deleteAvatar: () => deleteRequest<Me>("/users/me/avatar"),

  /**
   * GET /api/users/:id -> public profile
   */
  getPublic: (userId: number) => getRequest<PublicUser>(`/users/${userId}`),

  /**
   * GET /api/users/:id/matches?limit&offset -> { matches, limit offset }
   */
  listUserMatches: (userId: number, limit = 50, offset = 0) => {
    return getRequest<UserMatches>(`/users/${userId}/matches`, { limit, offset });
  },

  /**
   * GET /api/users/:id/stats
   */
  getStats: (userId: number) => getRequest<UserStats>(`/users/${userId}/stats`),

  /**
   * GET /api/users/search?q&limit&offset -> { users, limit, offset }
   */
  searchUser: (q: string, limit = 20, offset = 0) => {
    return getRequest<{ users: PublicUser[]; limit: number; offset: number }>(`/users/search`, { q, limit, offset });
  },
};
