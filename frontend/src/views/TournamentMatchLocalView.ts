...
export function TournamentMatchLocalView(root: HTMLElement) {
  ...
  const header = h("div", {
    class:
      "h-16 px-4 border-b border-emerald-100 bg-emerald-50/70 flex items-center justify-between",
  });
  const canvasWwrap = h("div", {
    class: "flex-1 min-h-0 grid place-items-center bg-emerald-50",
  });
  ...
  const quitBtn = h("button", {
    class:
      "px-6 py-2 bg-emerald-700 rounded rounded-lg flex flex-row gap-4 items-center text-white text-lg font-semibold hover:bg-emerald-400",
  });
  ...
  const state: MatchState = createGameLocalState(
    ...
    {
      id: configSettings.me.id,
      name: configSettings.me.pseudo,
      avatar_url: configSettings.me.avatar_url,
    },
    {
      id: configSettings.opponent!.id,
      name: configSettings.opponent!.pseudo,
      avatar_url: configSettings.opponent!.avatar_url,
    },
  ...
  const leftInfoName = h("span", {
    class: "font-bold",
    text: state.leftP.name,
  });
  const leftInfoScore = h("span", {
    class:
      "ml-2 font-bold text-lg text-emerald-700 bg-white rounded-lg min-w-10 text-center shadow",
    text: String(state.leftP.score),
  });
  mount(leftInfoWrap, leftInfoAvatar, leftInfoName, leftInfoScore);

  const rightInfoWrap = h("div", { class: "flex items-center gap-2" });
  const rightInfoScore = h("span", {
    class:
      "mr-2 font-bold text-lg text-emerald-700 bg-white rounded-lg min-w-10 text-center shadow",
    text: String(state.rightP.score),
  });
  const rightInfoName = h("span", {
    class: "font-bold",
    text: state.rightP.name,
  });
